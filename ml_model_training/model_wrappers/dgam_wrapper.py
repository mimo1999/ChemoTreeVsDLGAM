...
where ell indexes the daily lag bin and w_k(.) / f_k(.) / m_k(.) are
penalised smooths estimated by REML via mgcv's ``bam``.

Feature-name conventions (VMD/concatenate from ml_feature_matrix_builder.py)
-----------------------------------------------------------------------------
...
class DGAMClassifier(ClassifierMixin, BaseEstimator):
    ...
    dl_nonlinear : bool
        ...
        If False (default), a linear signal-regression term ``s(Lag, by=Value)``.
    k_lag : int
    ...
    max_labs : int
        Cap on the number of labs that receive a distributed-lag term.
    min_lags : int
        Minimum distinct bins a lab must have to earn a DL term.
    method : str
    ...
    def _parse(self, feature_names: list):
