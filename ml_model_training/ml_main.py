...
class MLTrainingPipeline:
    ...
    def train(self):
        ...
        for fold in range(self.num_folds):
            ...
            # Capture the pre-concat val split for IGANN early stopping. Only
            # useful when oversampling is active: the val split is from the real
            # (~4.85% positive) distribution, so IGANN's early stopping is judged
            # on the true class balance rather than the 50/50 oversampled split.
            igann_val_set = (
                (X_val.copy(), Y_val.copy())
                if self.model_type == 'IGANN' and self.oversampling_method == 'minority'
                else None
            )
            ...
            test_metrics, best_params = self.trainer.train_model(
                X_train, Y_train, X_test, Y_test, fold, subj_ids_train,
                sample_weight=sample_weight, val_set=igann_val_set
            )
...
def main():
    ...
    parser.add_argument('--top_k_features', type=int, default=250,
                        help='Number of top features to retain via tree-based importance '
                             'when --feature_selection 1 is set (default: 250)')
