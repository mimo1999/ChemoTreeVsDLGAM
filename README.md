...
| `IG-EBM` | Interaction-Guided EBM | EBM with FAST auto-search replaced by forced same-itemid, cross-day-bin interaction pairs (tests whether hand-specifying temporal interactions beats greedy search) | [`ig_ebm_wrapper.py`](ml_model_training/model_wrappers/ig_ebm_wrapper.py) |  
...
| `--feature_selection {0,1}` | boolean-as-int | Master switch for a two-tier pruning pipeline, applied before training. `1` is effectively required for the wide VMD/`A` matrices to keep the additive models' term count tractable. **Tier 1** (inside feature extraction): restricts to the top 100 lab itemids by historical importance (`MIMIC_IV/saved_data/top_features/feat_imp_summary.csv`) before any per-day/per-bin columns are even generated. **Tier 2** (after extraction): runs the `--selector_type` ranking method over the resulting columns and keeps the top `--top_k_features` of them. Both tiers are gated by this single flag — there is no way to run one without the other. |
| `--top_k_features K` | `int`, default `250` | Tier 2 only. Number of columns kept by the feature selector when `--feature_selection 1` is set. Bounds the number of shape functions/terms the explainable models have to fit (and, for GAM/DGAM, the PIRLS/REML design matrix size). Ignored when `--selector_type none`. |
| `--selector_type` | `none, tree, mi, correlation, xgb_gain, lasso` | Tier 2 only. Which ranking method picks the top-`k` columns: `tree` = RandomForest MDI importance (default), `mi` = mutual information, `correlation` = \|Pearson r\| with the target, `xgb_gain` = XGBoost gain importance, `lasso` = L1-logistic-regression coefficients, `none` = passthrough (skips Tier 2, keeping every column that survived Tier 1). Different explainable models respond differently to *which* features survive selection, so this is swappable per experiment rather than hard-coded to one ranking method. |
